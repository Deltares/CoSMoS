...
Meteo collection
------------
CoSMoS automatically downloads, collects and writes meteorological data for each model based on user-defined settings in the scenario file (see :ref:`Scenario <scenario>`). 
Meteorological forcing data can be found in the *meteo* folder, for example:

.. include:: examples/meteo_folder.txt
       :literal: 

There are three types of meteo data, which are elaborated in the sections below:
...
- **Ensemble of tracks** (keywword: *<track_ensemble>* in scenario file). The *hurricane_irma* folder is an example of an ensemble of tracks (tracks are generated based on the *.cyc* file).
...
The keyword *<ensemble_nrtracks>* in the scenario file prescribes the number of tracks that need to be generated.
