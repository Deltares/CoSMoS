...
const onEachFeatureETWL = function (feature, layer) {

  const html = 'TWL : &#9;' + feature.properties.TWL + ' m above MSL &#9;' + '<br />' +
               'HAT : &#9;' + feature.properties.HAT + ' m above MSL &#9;' + '<br />' +
               'TWL - HAT : &#9;' + feature.properties.TWLminusHAT + ' m &#9;' + '<br />'
  layer.bindTooltip(L.tooltip({ direction: 'top' }).setContent(html));

  var popup = L.popup({"maxWidth": "100%"});
  wlurl = 'html/estimated_total_water_level_timeseries.html?'
  wlurl = wlurl +       `name=${feature.properties.station_name}`
  wlurl = wlurl + '&' + `longname=${feature.properties.station_name}`
  wlurl = wlurl + '&' + `id=${feature.properties.id}`
  wlurl = wlurl + '&' + `cycle=${currentCycle}`
  wlurl = wlurl + '&' + `duration=${currentScenario["duration"]}`
  wlurl = wlurl + '&' + `model_name=${feature.properties.model_name}`
...
// Format for the points:
const pointToLayerETWL = function (feature, latlng) {
//	console.log(latlng)
  return new L.CircleMarker(latlng, {
    radius: 5,
    fillOpacity: 0.7,
	opacity: 0,
    color: thresholdsRUNUP(feature.properties.TWLminusHAT)
  });
};


function makeEstimatedTotalWaterLevelLayer() {
  // Add point markers for nearshore extremes
  console.log("MakingETWL")
  var lyr = L.geoJson(undefined, {
    pointToLayer: pointToLayerETWL,
    onEachFeature: onEachFeatureETWL
  })
  layers["estimated_total_water_level"].push(lyr);
  var pth = "data/" + currentScenario["name"] + "/" + currentCycle + "/"
  loadjs(pth + "estimated_total_water_level.geojson.js", etwlLoaded, etwlFailed);
}

function etwlLoaded() {
  layers["estimated_total_water_level"][0].addData(etwl);
}

function etwlFailed() {
	console.log('Could not load estimated_total_water_level.geojson.js');
}
